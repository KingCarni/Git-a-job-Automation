...
const fixturePath = path.join(__dirname, 'JOB-56-fixtures.json');

function readFixture() {
  return JSON.parse(fs.readFileSync(fixturePath, 'utf8'));
}

test.describe('JOB-56 generated QA fixture validation', () => {
  test('JOB-56-FIXTURE-001 - fixture file has required structure', async () => {
    const data = readFixture();

    expect(data.issueKey).toBe('JOB-56');
    expect(data.feature).toBeTruthy();
    expect(Array.isArray(data.cases)).toBeTruthy();
    expect(data.cases.length).toBeGreaterThan(0);
  });

  test('JOB-56-FIXTURE-002 - generated cases have IDs and expected outcomes', async () => {
    const data = readFixture();
  ...
  test('JOB-56-FIXTURE-003 - daily bonus rules are safe when applicable', async () => {
    const data = readFixture();
