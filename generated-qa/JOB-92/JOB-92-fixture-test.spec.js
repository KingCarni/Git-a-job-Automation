...
const fixturePath = path.join(__dirname, 'JOB-92-fixtures.json');

function readFixture() {
  return JSON.parse(fs.readFileSync(fixturePath, 'utf8'));
}

test.describe('JOB-92 generated QA fixture validation', () => {
  test('JOB-92-FIXTURE-001 - fixture file has required structure', async () => {
    const data = readFixture();

    expect(data.issueKey).toBe('JOB-92');
  ...
  test('JOB-92-FIXTURE-002 - generated cases have stable IDs and expected outcomes', async () => {
    const data = readFixture();
  ...
  test('JOB-92-FIXTURE-003 - generated rule references exist in fixture rules', async () => {
    const data = readFixture();
